...
all_dailies$V1 = weekdays(all_dailies$V1)

all_dailies = data.table(all_dailies)[,list(V3=mean(V3)),by=c("V1","V2")]

all_dailies$V1 = factor(all_dailies$V1, levels = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"))
...
ggsave(filename = "/tmp/fitbit-dow-plot.png",
       plot = plot,
       scale = 1,
